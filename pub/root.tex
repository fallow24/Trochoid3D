...
\author{Fabian Arzberger$^{1}$, Fabian Wiecha$^{1}$, Jasper Zevering$^{1}$, Julian Rothe$^{2}$,\\Dorit Borrmann$^{3}$, Sergio Montenegro$^{2}$, and Andreas N{\"u}chter$^{1}$% <-this % stops a space
...
\input{sections/02-related.tex}
\input{sections/03-calibration.tex}
\input{sections/04-model.tex}
